...
func Test_QueryParser(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "JSONB query",
			qry:       `payload->'data'->'manifest'->'metadata'->'labels'->>'foo' = 'bar'`,
			qryParser: NewQueryParser("payload"),
			outQry:    "payload -> 'data' -> 'manifest' -> 'metadata' -> 'labels' ->> 'foo' = ?",
		...
		},
		{
			...
			qry: `payload->'data'->'manifest'->'metadata'->'labels'->>'foo' = 'bar' and ` +
				`( payload->'data'->'manifest' ->> 'foo' in ('value1', 'value2') or ` +
				`payload->'data'->'manifest'->>'labels' <> 'foo1')`,
			qryParser: NewQueryParser("payload"),
			outQry: "payload -> 'data' -> 'manifest' -> 'metadata' -> 'labels' ->> 'foo' = ? and " +
				"(payload -> 'data' -> 'manifest' ->> 'foo' in( ? , ?) or " +
				"payload -> 'data' -> 'manifest' ->> 'labels' <> ?)",
			outValues: []interface{}{"bar", "value1", "value2", "foo1"},
			wantErr:   false,
		},

		{
			name:      "JSONB function query",
			qry:       `payload->'data'->'manifests'@>'[{"metadata":{"labels":{"foo":"bar"}}}]'`,
			qryParser: NewQueryParser("payload"),
			outQry:    "payload->'data'->'manifests'@>'[{\"metadata\":{\"labels\":{\"foo\":\"bar\"}}}]'",
			outValues: nil,
			wantErr:   false,
		},
